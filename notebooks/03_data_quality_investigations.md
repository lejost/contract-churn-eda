...
Four things looked like droppable noise. Each was profiled *before* removal. The discipline: **look at the records you're about to delete, because `null ≠ zero` and `anomalous ≠ droppable`.**
...
# Profile the categorical fields across the WHOLE negative population,
# not one slice — a single slice is how you reach a wrong conclusion.
for c in ["type_field", "status_field"]:
    neg.groupBy(f"`{c}`").count().orderBy(F.desc("count")).show(truncate=False)
```

**Finding (illustrative):** the negatives concentrate on a specific accounting pattern that nets out — not a genuine decline.

**Consequence:** a naive `sum(value)` would let the negative leg cancel real volume and manufacture a fake signal. Prefer netting the matched pair, or "max value ever held."
...
**Lesson:** picking the correct natural key turned a scary "duplicate rate" into zero. The problem was the key, not the data.
