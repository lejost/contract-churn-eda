...
Goal: decide whether a candidate grouping key is safe to use as the unit of
analysis, or whether it fragments / leaks. Measure where it breaks instead of
picking on intuition.

Generic schema: each record has an entity_id, a candidate group_key, a finer
item_id, and a value.

Decision: the key is usable for the overwhelming majority; rare leaky cases
carry a flag so each downstream feature decides whether to exclude them.
...
df = (
    df.join(
        leaky.select("`group_key`").withColumn("is_leaky_key", F.lit(True)),
        on="group_key",
        how="left",
    )
